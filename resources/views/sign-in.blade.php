<?php
$model = auth()->user();
if($model->mobile_verification_code){
    $model->update([
...
<div class="h-full">
    <div class="px-view py-view mx-auto">
        ...
        <form id="authenticate_form" class="bg-white shadow rounded-lg p-8 max-w-login mx-auto" method="POST" action="{{ route('nova-two-factor.auth') }}">
            ...
            <h2 class="text-xl text-center font-normal mb-6 text-90">Google Two factor authentication</h2>
            ...
            <div class="flex mb-6">
                @if($model->sms_two_fa_enabled)
                <div class="justify-start">
                    <a class="text-primary dim" href="{{ route('nova-two-factor.viasms') }}">
                        Send via SMS
                    </a>
                </div>
                @endif
